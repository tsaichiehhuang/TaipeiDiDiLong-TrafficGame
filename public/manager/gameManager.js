...
class GameManager {
    ...
    setup = () => {
        ...
        // Make background
        let yPos =
            (-this._bgImages[0].height - this.cameraYOffest) / 2 + height + 10;
        let longBgGroup = new Group();
        for (let i = 0; i < this._bgImages.length; i++) {
            this._createRoadSprite(
                longBgGroup,
                width / 2,
                yPos,
                this._bgImages[i],
    ...
    showBackground() {
        ...
        this._bgSprites.forEach((sprite) => {
            if (
                sprite.position.y + sprite.h / 2 >= topY &&
                sprite.position.y - sprite.h / 2 <= bottomY
            ) {
                sprite.draw();
            }
        });
     ...
     */
    getVisibleYRange = () => {
        return [camera.position.y - height / 2, camera.position.y + height / 2];
    };
    ...
    getGameYRange = () => {
        return [
            this._bgSprites[this._bgSprites.length - 1].position.y -
                this._bgSprites[this._bgSprites.length - 1].h / 2,
    ...
    _checkIfShouldNextSection = () => {
        ...
        if (
            ...
            this._nextSectionStartY - height * this._nextSectionHeightCount
        ) {
    ...
    _repositionRoadsIfNeed = () => {
        ...
        if (player.vel.y < 0) {
            this._roadSprites.forEach((roadSprite, i) => {
                ...
                if (!isVisible) {
                    ...
                    roadSprite.position.y =
                        this._roadSprites[nextIndex].position.y -
                        this._roadHeight;
                }
            });
            ...
            this._roadSprites.forEach((roadSprite, i) => {
                ...
                if (!isVisible) {
                    let prevIndex =
                        (i - 1 + this._roadSprites.length) %
                        this._roadSprites.length;
                    roadSprite.position.y =
                        this._roadSprites[prevIndex].position.y +
                        this._roadHeight;
                }
            });
        }
