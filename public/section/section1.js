    ...
    return {
        ...
        onSectionStart: () => {
            ...
            eventManager.listen(EVENT_LEVEL_TRAFFIC_LIGHT, (status) => {
                ...
                switch (status) {
                    case EventStatus.START:
                        trafficLightY = playerController.getPlayer().position.y - 1150
            ...
            eventManager.listen(EVENT_QA_FLOWER_SELLER, (status) => {
                ...
                switch (status) {
                    case EventStatus.START:
                        ...
                        setTimeout(() => {
                            questionManager.getRandomQuestion(EVENT_QA_FLOWER_SELLER)
                            showQaQuestion = true
                        }, 3500)
                        break
                    case EventStatus.SUCCESS:
        ...
        draw: () => {
            ...
            if (showTrafficLight) {
                ...
                image(trafficLightImg, gameManager.getRoadXRange()[1] / 2 + 25, trafficLightY - 50, 375, 375)
            }

            // trigger red line parking img
            image(this._redLineVio, gameManager.getRoadXRange()[0] + 8, startPosiY - 900)
            ...
            if (gameManager.getSection() == 1) {
                ...
                if (currentEvents.has(EVENT_REPORT_RED_LINE_PARKING)) {
                    if (keyIsDown(32)) {
                        ...
                        eventManager.startEvent(EVENT_LEVEL_TRAFFIC_LIGHT, 6500)
                    } else if (
                ...
                if (currentEvents.has(EVENT_LEVEL_TRAFFIC_LIGHT)) {
                    ...
                    if (isPlayerStopped) {
                        if (
                            playerController.getPlayer().position.y + 50 < trafficLightY + 550 &&
                            playerController.getPlayer().position.y + playerController.getPlayer().height / 2 < 50
                        ) {
                            eventManager.successEvent(EVENT_LEVEL_TRAFFIC_LIGHT)
                        }
                        if (playerController.getPlayer().position.y + 50 < trafficLightY + 370) {
                            eventManager.failEvent(EVENT_LEVEL_TRAFFIC_LIGHT)
                        }
                    } else if (playerController.getPlayer().position.y + 50 < trafficLightY + 370) {
                        eventManager.failEvent(EVENT_LEVEL_TRAFFIC_LIGHT)
                    }
                }
                ...
                if (showTrafficLightText) {
                    trafficLightManager.draw(isStoppedInRedLight ? 'stopped' : 'notStopped')
                ...
                if (showQaQuestion) {
                    questionManager.showQuestion({
                        greeting: '來呦，買一朵玉蘭花吧！ 蝦米？你叫我走開？ 那先回答我的問題吧！',
                    })
                }
