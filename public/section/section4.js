const Section4 = () => {
    ...
    return {
        preload: () => {
            ...
            this._buyDinnerSuccess = loadImage("../images/section4/Buy_food.png");
        },
            ...
            eventManager.listen(EVENT_LEVEL_BUY_DINNER, (status) => {
                ...
                switch (status) {
                    case EventStatus.START:
                        ...
                        startPosiY_buyDinner = playerController.getPlayer().position.y;
                        mainUIController.setTaskText("請將車子停入右方停車格，完成購買便當的任務！");
                        break;
                    case EventStatus.SUCCESS:
                        // Do something
        ...
        draw: () => {
            ...
            image(this._parkingSpace, gameManager.getRoadXRange()[1] - 10, startPosiY_buyDinner - 900);
            ...
            if(gameManager.getSection() == 4) {
                ...
                if (currentEvents.has(EVENT_LEVEL_BUY_DINNER)) {
                    ...
                    // 限制玩家移動範圍 - 上方
                    if (playerController.getPlayer().position.y < startPosiY_buyDinner - 580 && playerController.getPlayer().position.x > gameManager.getRoadXRange()[1] &&
                        playerController.getPlayer().position.x < gameManager.getStreetXRange()[1]) {
                        playerController.getPlayer().position.y = startPosiY_buyDinner - 580;
                        sparkController.createSpark(playerController.getPlayer().position.x, startPosiY_buyDinner - 650);
                    }
                    // 限制玩家移動範圍 - 下方
                    if (playerController.getPlayer().position.y + playerController.playerHeight > startPosiY_buyDinner - 330 && playerController.getPlayer().position.x > gameManager.getRoadXRange()[1] &&
                        playerController.getPlayer().position.x < gameManager.getStreetXRange()[1]) {
                        playerController.getPlayer().position.y = startPosiY_buyDinner - 330 - playerController.playerHeight;
                        sparkController.createSpark(playerController.getPlayer().position.x, startPosiY_buyDinner - 400);
                    }

                    // 判斷有沒有停在格子內
                    if (playerController.getPlayer().position.y > startPosiY_buyDinner - 580 &&
